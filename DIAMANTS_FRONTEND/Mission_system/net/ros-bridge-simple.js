...
export class RosWebBridge {
    constructor({ url = DEFAULT_URL, autoConnect = false, silent = true } = {}) {
        ...
        /** @type {'simulation'|'real'|'sitl'|'unknown'} */
        this.mode = 'unknown';
    ...
    _onMessage(raw) {
        ...
        if (type === 'initial_state' || type === 'current_status') {
            this.state = data;
            // Detect mode from bridge or gateway
            const sysMode = data?.system?.mode || msg.mode;
            if (sysMode) {
                this.mode = sysMode;
                this._log('log', `📡 Mode détecté: ${sysMode}`);
                try {
                    window.dispatchEvent(new CustomEvent('diamants:mode-change', { detail: { mode: sysMode } }));
                } catch (_) { /* safe */ }
            }
        ...
        // Fractal hypervision stack (L0-L5, couche basse ROS)
        if (type === 'fractal_state') {
            try {
                window.dispatchEvent(new CustomEvent('diamants:fractal-state', {
                    detail: data,
                }));
            } catch (_) { /* safe */ }
            return;
        ...
        // AI decision from autonomous system
        if (type === 'ai_decision') {
            const droneId = msg.drone_id || data.drone_id || 'agent_1';
            try {
                window.dispatchEvent(new CustomEvent('diamants:ai-decision', {
                    detail: { drone_id: droneId, ...data },
                }));
            } catch (_) { /* safe */ }
            return;
        }

        // AI perception from autonomous system
        if (type === 'ai_perception') {
            const droneId = msg.drone_id || data.drone_id || 'agent_1';
            try {
                window.dispatchEvent(new CustomEvent('diamants:ai-perception', {
                    detail: { drone_id: droneId, ...data },
                }));
            } catch (_) { /* safe */ }
            return;
    ...
    _scheduleReconnect() {
        ...
        if (this._reconnectRetries > RECONNECT_MAX_RETRIES) {
            ...
            try { window.dispatchEvent(new CustomEvent('diamants:ws-gave-up')); } catch (_) {}
            return;
        }
        this._log('log', `🔄 Reconnecting in ${this._reconnectDelay / 1000}s… (attempt ${this._reconnectRetries}/${RECONNECT_MAX_RETRIES})`);
