// Fragment shader ULTRA QUALITY - bruns renforcés + diversité + punch
export default `
...
void main() {
    ...
    // Sélection du type d'herbe avec biais brun (plus de bases foncées)
    if (random < 0.15) {
    ...
    if (random2 > 0.45) {
        ...
        finalTipColor = mix(finalTipColor, mixTip, 0.35);
        finalBaseColor = mix(finalBaseColor, mixBase, 0.45); // base plus brune
    }
    
    // ===== GRADIENT VERTICAL AVEC SOL BRUN =====
    float elevationNorm = clamp(vElevation * 0.5, 0.0, 1.0);
    vec3 grassColor = mix(finalBaseColor, finalTipColor, pow(elevationNorm, 0.55));
    
    // ===== SOL ULTRA-BRUN À LA BASE =====
    float soilRandom = fract(sin(dot(vPosition.xz * 2.1, vec2(45.164, 23.431))) * 43758.5453);
    
    vec3 richBrownSoil = vec3(0.40, 0.26, 0.15);       // Terre riche brune
    vec3 darkBrownSoil = vec3(0.28, 0.17, 0.10);       // Terre sombre
    vec3 claySoil     = vec3(0.44, 0.32, 0.21);       // Argile/ocre
    
    vec3 soilColor = mix(mix(richBrownSoil, claySoil, 0.5), darkBrownSoil, soilRandom);
    
    // Zone de transition sol-herbe plus large pour ULTRA
    float soilFactor = 1.0 - pow(elevationNorm, 0.35); // base plus large
    grassColor = mix(grassColor, soilColor, soilFactor * 0.85);
    
    // ===== EFFETS ULTRA: ANIMATION TEMPORELLE =====
    float timeEffect = sin(uTime * 0.0012 + vPosition.x * 0.12 + vPosition.z * 0.08) * 0.10 + 1.0;
    grassColor *= timeEffect; // léger scintillement organique
    
    // ===== GRADIENT LATÉRAL AVEC BRUN =====
    float sideEffect = pow(vSideGradient, 0.85);
    grassColor = mix(grassColor, soilColor * 1.12, sideEffect * 0.45);
    ...
    // ===== RENDU FINAL ULTRA AVEC SIGNATURE VISUELLE =====
    vec3 finalColor = grassColor * NdotL * 1.25; // boost ULTRA

    // Signature ULTRA: saturation et contraste plus élevés
    float lum = dot(finalColor, vec3(0.299, 0.587, 0.114));
    finalColor = mix(vec3(lum), finalColor, 1.65);
    finalColor = pow(finalColor, vec3(1.0/1.85));
