...
// ============= DEFORMATION FUNCTION (EXACTE DU REPO) =============
vec3 deform(vec3 pos) {
    ...
    // Gentle sway (mouvement doux augmenté)
    float gentleSway = sin(uTime * uSpeed * 1.2 + hash * 10.0) * 0.15;
    float gentleSwayZ = cos(uTime * uSpeed * 0.9 + hash * 15.0) * 0.08;
    vec3 gentleDir = normalize(vec3(1.0, 0.0, 0.5));
    vec3 gentleOffset = gentleDir * gentleSway * t;
    gentleOffset.z += gentleSwayZ * t;

    // Strong wind (vent plus prononcé)
    vec3 worldPos = (instanceMatrix * vec4(pos, 1.0)).xyz;
    float wave = cnoise(worldPos.xz * 0.25 + vec2(uTime * uSpeed * 0.3, 0.0));
    float wave2 = cnoise(worldPos.xz * 0.15 + vec2(0.0, uTime * uSpeed * 0.2));
    float strongWind = (wave + wave2 * 0.5) * 0.4;
    vec3 strongDir = normalize(vec3(0.3, 0.0, 1.0));
    float y = pos.y;
    vec3 strongOffset = strongDir * strongWind * pow(y, 1.8);
    ...
    // 4x4 par vertex, sur ~1,2 M de vertices par frame.
...
void main() {
    ...
    // Calculée AVANT le fondu : celui-ci est une homothétie, elle ne doit pas
    // altérer l'éclairage des brins de bordure.
    vec3 normalWS = normalize(cross(offsetX - p, offsetY - p));

    // ===== FONDU GÉOMÉTRIQUE DE BORDURE =====
    // Le brin rapetisse progressivement jusqu'à zéro au lieu que son chunk
    // entier disparaisse d'un bloc — c'est ça, l'effet "carré d'herbe".
