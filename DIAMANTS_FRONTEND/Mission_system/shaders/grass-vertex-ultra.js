// Vertex shader ULTRA QUALITY - basé sur la version stable, avec vent amplifié et rafales
export default `
uniform float uTime;
...
// ULTRA deform with stronger wind and gusts
vec3 deform(vec3 pos) {
    ...
    float bendStrength = mix(0.45, 0.9, hash);      // ++ compared to stable
    float bendStart = mix(0.0, 0.25, hash);
    float t = clamp((pos.y / 2.0 - bendStart) / (1.0 - bendStart), 0.0, 1.0);
    float topBendFactor = bezier(t, 0.18);          // ++ curvature

    // Gentle sway (stronger)
    float gentleSway = sin(uTime * uSpeed * 1.8 + hash * 12.0) * 0.28; // ++
    float gentleSwayZ = cos(uTime * uSpeed * 1.4 + hash * 18.0) * 0.16; // ++
    vec3 gentleDir = normalize(vec3(1.0, 0.0, 0.55));
    vec3 gentleOffset = gentleDir * gentleSway * t;
    gentleOffset.z += gentleSwayZ * t;

    // Strong wind (significantly stronger)
    vec3 worldPos = (instanceMatrix * vec4(pos, 1.0)).xyz;
    float wave = cnoise(worldPos.xz * 0.30 + vec2(uTime * uSpeed * 0.45, 0.0));
    float wave2 = cnoise(worldPos.xz * 0.18 + vec2(0.0, uTime * uSpeed * 0.35));
    float strongWind = (wave + wave2 * 0.6) * 0.85; // ++ amplitude
    vec3 strongDir = normalize(vec3(0.35, 0.0, 1.0));
    float y = pos.y;
    vec3 strongOffset = strongDir * strongWind * pow(y, 2.1); // ++ power

    // Gusts (sporadic bursts)
    float gust = max(0.0, sin(uTime * 0.35 + worldPos.x * 0.05 + worldPos.z * 0.04));
    gust = smoothstep(0.8, 1.0, gust);
    vec3 gustOffset = normalize(vec3(0.6, 0.0, 0.8)) * (0.35 * gust) * t * (0.6 + 0.4 * hash);

    // Apply
    localPosition += instanceZ * bendStrength * topBendFactor;
    localPosition += gentleOffset;
    localPosition += strongOffset;
    localPosition += gustOffset;
