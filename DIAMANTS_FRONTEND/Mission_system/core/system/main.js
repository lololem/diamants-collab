...
// Test Console Ninja avec des logs périodiques
setInterval(() => {
    console.log('💡 Console Ninja Test:', new Date().toLocaleTimeString());
    console.log('🔄 DIAMANTS Status: Active');
}, 5000);
...
async function waitForTHREE() {
    return new Promise((resolve, reject) => {
        if (window.THREE && window.THREE_READY) {
            console.log('✅ THREE.js déjà prêt !');
        ...
        window.addEventListener('threeReady', (event) => {
            console.log('✅ THREE.js maintenant prêt !');
...
async function initializeTHREE() {
    try {
        ...
        console.log('✅ THREE.js bootstrap terminé, récupération des composants...');
        ...
        try {
            ...
            console.warn('⚠️ Post-processing non disponible:', postProcessError);
        }
        
        console.log('✅ Tous les composants THREE.js chargés');
        return true;
    } catch (error) {
...
import { makeOdometry, yawToQuat } from '../../net/ros-bridge.js';
import DiamantRosManager from '../../net/adaptive-websocket-manager.js';
...
class DiamantsMissionSystem {
    constructor() {
        ...
        this.ros = null;
        this.rosConnected = false; // État de connexion ROS
        ...
        console.log('🚀 Initialisation DIAMANTS Mission System V3 avec EZ-Tree');
        this.init();
    }
    ...
    async init() {
        ...
        // Créer la scène en premier pour IntegratedController
        await this.setupScene();
        
        // Initialiser le contrôleur intégré AVANT WebGL pour permettre ROS sans 3D
        await this.setupIntegratedController(); // NOUVEAU: Contrôleur intégré (indépendant de WebGL)
        
        await this.setupRenderer();
        // If renderer couldn't be created (WebGL blocked/unavailable), stop early for visual components only
        if (!this.renderer) {
            console.error('🛑 WebGL renderer unavailable. Continuing with ROS controller only.');
            // Le contrôleur ROS peut fonctionner sans WebGL, on continue sans la partie visuelle
        ...
        console.log('✅ DIAMANTS Mission System V3 initialisé avec succès');
    }
    ...
    async setupRenderer() {
        ...
        try {
            ...
            console.log('✅ WebGL2 context created with safe attributes');
        } catch (e) { 
            console.warn('WebGL2 safe failed:', e.message);
        }
        ...
        if (!gl) {
            try {
                ...
                console.log('✅ WebGL2 context created with minimal attributes');
            } catch (e) { 
                console.warn('WebGL2 minimal failed:', e.message);
            }
        }
        ...
        if (!gl) {
            try {
                ...
                console.log('✅ WebGL1 context created with safe attributes');
            } catch (e) { 
                console.warn('WebGL1 safe failed:', e.message);
            }
        }
        ...
        if (!gl) {
            try {
                ...
                console.log('✅ WebGL1 context created with minimal attributes');
            } catch (e) { 
                console.warn('WebGL1 minimal failed:', e.message);
            }
        }
        ...
        if (!gl) {
            try {
                ...
                console.log('✅ Experimental WebGL context created');
            } catch (e) { 
    ...
    const onContextLost = (e) => {
            ...
            console.warn('⚠️ WebGL context LOST - pausing rendering to avoid browser blocking');
    ...
    this.renderer.domElement.addEventListener('webglcontextrestored', this._onWebglContextRestored, false);
        ...
        console.log('🎨 Renderer configuré (safe defaults)');
    ...
    async setupRosWeb() {
        try {
            // 🔄 NOUVEAU: Gestionnaire WebSocket adaptatif DIAMANTS
            this.ros = new DiamantRosManager({
                debug: true,
                autoConnect: true,
                onConnect: () => {
                    console.log('🚁 DIAMANTS Bridge connecté - Animation des hélices activée');
                    this.rosConnected = true;
                    if (this.integratedController) {
                        this.integratedController.updateMetrics();
                    }
                },
                onDisconnect: () => {
                    console.log('⚠️ DIAMANTS Bridge déconnecté - Mode dégradé');
            ...
            console.log(`🔌 ROS2 WebSocket Setup: Gestionnaire adaptatif démarré`);
            ...
            this.drones.forEach((d) => {
                ...
                if (!window.SILENT_MODE) {
                    console.log(`📡 Configuration ROS2 pour drone ${d.id}: Topics ${ns}/cmd/*`);
                }
                
                // Subscribe to target pose commands avec le nouveau gestionnaire
                this.ros.subscribe(`${ns}/cmd/target_pose`, 'geometry_msgs/PoseStamped', (msg) => {
                    try {
                        ...
                        console.log(`📥 ${d.id} | ROS2 TARGET_POSE: [${p.x.toFixed(2)}, ${p.y.toFixed(2)}, ${p.z.toFixed(2)}] | État: ${previousState} -> ${d.state}`);
                ...
                this.ros.subscribe(`${ns}/cmd/land`, 'std_msgs/Empty', () => {
                    try { 
                        const previousState = d.state;
                        d.state = 'LANDING';
                        console.log(`📥 ${d.id} | ROS2 LAND COMMAND: État ${previousState} -> LANDING`);
                ...
                this.ros.subscribe(`${ns}/cmd/takeoff`, 'std_msgs/Float32', (msg) => {
                    try {
                        ...
                        console.log(`📥 ${d.id} | ROS2 TAKEOFF COMMAND: altitude=${targetAlt}m`);
            ...
            console.log('🌐 ROS-Web bridge prêt');
        } catch (e) {
            console.warn('ROS-Web non disponible:', e.message);
        }
    }
    ...
    async setupControls() {
        ...
        console.log('🎮 Contrôles configurés (autoRotate DÉSACTIVÉ)');
    }
    ...
    console.log('🎭 Post-processing configuré (désactivé par défaut)');
    }

    async setupEnvironment() {
        console.log('🌍 Création environnement provençal authentique...');
        ...
        console.log('✅ Environnement provençal prêt');
    }

    async setupIntegratedController() {
        console.log('🎛️ Initialisation contrôleur intégré...');
        ...
        try {
            ...
            try {
                ...
                window.DIAMANTS.toggleFollowMode = () => {
                    ...
                    console.log('ℹ️ Follow mode non implémenté dans cette build');
                };
            ...
            console.log('✅ Contrôleur intégré initialisé avec succès');
            ...
            try {
                ...
                // Dispatch engine ready event for diagnostic system
                window.dispatchEvent(new CustomEvent('diamants:engine-ready', { 
                    detail: { 
                        engineInstance: this.integratedController,
                        timestamp: new Date().toISOString()
                    } 
                }));
                if (!window.SILENT_MODE) {
                    console.log('📡 Event diamants:engine-ready dispatched');
                }
            } catch (_) { /* noop */ }
    ...
    exposeGlobalBridge() {
        try {
            ...
            console.log('🔗 Pont global window.DIAMANTS exposé (compat UI)');
        } catch (_) { /* noop */ }
    }

    setupDrones() {
        // Les drones sont maintenant créés et gérés par l'IntegratedController
        // pour éviter les doublons et avoir une gestion cohérente
        console.log('🚁 Les drones seront créés par l\'IntegratedController');
    ...
    onKeyDown(event) {
        switch (event.code) {
            ...
            case 'KeyA':
                ...
                console.log('🚫 AutoFocus DÉSACTIVÉ DÉFINITIVEMENT - contrôle manuel uniquement');
                this.autoFocus = false; // Force toujours false
                break;
            case 'KeyP':
                // Toggle post-processing
                this.usePostProcessing = !this.usePostProcessing;
                console.log(`🎭 Post-processing ${this.usePostProcessing ? 'activé' : 'désactivé'}`);
                break;
            case 'KeyG':
                // Toggle fog (brume)
                if (this.scene.fog) {
                    this.scene.fog = null;
                    console.log('🌫️ Brume désactivée');
                } else {
                    this.scene.fog = new THREE.FogExp2(0xE6E6FA, 0.0008);
                    console.log('🌫️ Brume activée');
                }
                break;
...
Souris: Navigation 3D
    ...
    animate() {
        ...
        // 🚀 PERFORMANCE OPTIMIZATION - Mesure temps d'exécution
        ...
        // 🎛️ INTELLIGENT THROTTLING - Limiter deltaTime pour éviter les pics
        const throttledDelta = Math.min(delta, 0.033); // Max 30fps equivalent
        ...
        // 🎯 SYSTÈME INTELLIGENT DE MISE À JOUR avec throttling adaptatif
        if (this.integratedController) {
            const controllerStart = performance.now();
            
            // Utiliser le deltaTime throttled pour éviter les pics
            this.integratedController.update(throttledDelta);
            
            const controllerTime = performance.now() - controllerStart;
            
            // 📊 Debug performance controller uniquement si trop lent
            if (controllerTime > 20) { // Plus de 20ms = problème
                if (typeof debugLog !== 'undefined') {
                    debugLog('ANIMATION', 'WARN', `Controller update lent: ${controllerTime.toFixed(1)}ms`, {
                        deltaTime: throttledDelta,
    ...
    if (!this.renderer || this._contextLost) {
        ...
        if (frameTime > 16.67) { // Plus que 60fps
            // Seulement log tous les 60 frames pour éviter spam
            if (this.frameCounter % 60 === 0) {
                if (typeof debugLog !== 'undefined') {
                    let level = 'INFO';
                    let message = `Performance: ${frameTime.toFixed(1)}ms/frame`;
                    
                    if (frameTime > 33) { // Moins de 30fps
                        level = 'WARN';
                        message = `🐌 Performance lente: ${frameTime.toFixed(1)}ms (target: <16ms)`;
                    }
                    
                    debugLog('ANIMATION', level, message, {
                        frameTime: frameTime,
    ...
    lockCamera() {
        ...
        console.log('🔒 Caméra VERROUILLÉE en position actuelle');
    }

    unlockCamera() {
        // Déverrouille la caméra
        this.isLocked = false;
        console.log('🔓 Caméra DÉVERROUILLÉE - contrôles libres');
    }

    launchMission() {
        console.log('🚀 Démarrage mission DIAMANTS');
        
        if (!this.integratedController) {
            console.error('❌ IntegratedController non initialisé');
            return;
        }

        try {
            // Démarrer la mission de scouting collaboratif
            this.integratedController.startInitialMission();
            console.log('✅ Mission lancée avec succès');
        } catch (error) {
            console.error('❌ Erreur lors du lancement de mission:', error);
        }
    }
    ...
    emergencyLand() {
        ...
        if (this.integratedController && this.integratedController.emergencyStop) {
            this.integratedController.emergencyStop();
            console.log('✅ Arrêt d\'urgence exécuté');
        } else {
            console.warn('⚠️ Fonction d\'arrêt d\'urgence non disponible');
        }
    }
    ...
    dispose() {
        ...
        console.log('🧹 DIAMANTS Mission System nettoyé');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        console.log('🎯 DIAMANTS Mission System V3 démarré avec EZ-Tree authentique');
    } catch (error) {
